...
public class GameControllar : MonoBehaviour {
	...
	void Update(){
		...
		if (count == 0){
			Instantiate(Target[Random.Range (0, Target.Length)],ps1.position,transform.rotation);
			Instantiate(Target[Random.Range (0, Target.Length)],ps2.position,transform.rotation);
			Instantiate(Target[Random.Range (0, Target.Length)],ps3.position,transform.rotation);
